![build](https://github.com/caa-assistant/github-action/workflows/build/badge.svg)

# Handling CAAs and DCOs via GitHub Action

Streamline your workflow and let this GitHub Action (a lite version of [CAA Assistant](https://github.com/caa-assistant/caa-assistant)) handle the legal side of contributions to a repository for you. CAA assistant GitHub action enables contributors to sign CAAs from within a pull request. With this GitHub Action we could get rid of the need for a centrally managed database by **storing the contributor's signature data** in a decentralized way - **in the same repository's file system** or **in a remote repository** which can be even a private repository.

### Features
1. decentralized data storage
1. fully integrated within github environment
1. no User Interface is required
1. contributors can sign the CAA or DCO by just posting a Pull Request comment
...
#### 1. Add the following Workflow File to your repository in this path`.github/workflows/caa.yml`
...
jobs:
  CAAAssistant:
    ...
    steps:
      - name: "CAA Assistant"
        if: (github.event.comment.body == 'recheck' || github.event.comment.body == 'I have read the CAA Document and I hereby sign the CAA') || github.event_name == 'pull_request_target'
        uses: contributor-assignment/github-action@v2.6.1
        env:
        ...
        with:
          path-to-signatures: 'signatures/version1/caa.json'
          path-to-document: 'https://github.com/caa-assistant/github-action/blob/master/SAPCAA.md' # e.g. a CAA or a DCO document
         ...
         # the followings are the optional inputs - If the optional inputs are not given, then default values will be taken
          ...
          #create-file-commit-message: 'For example: Creating file for storing CAA Signatures'
          #signed-commit-message: 'For example: $contributorName has signed the CAA in $owner/$repo#$pullRequestNo'
          #custom-notsigned-prcomment: 'pull request comment with Introductory message to ask new contributors to sign'
          #custom-pr-sign-comment: 'The signature to be committed in order to sign the CAA'
          #custom-allsigned-prcomment: 'pull request comment when all contributors has signed, defaults to **CAA Assistant Lite bot** All Contributors have signed the CAA.'
          #lock-pullrequest-aftermerge: false - if you don't want this bot to automatically lock the pull request after merging (default - true)
          #use-dco-flag: true - If you are using DCO instead of CAA

```

##### Demo for step 1

![add-caa-file](https://github.com/caa-assistant/github-action/blob/master/images/adding-caafile.gif?raw=true)

#### 2. Pull Request event triggers CAA Workflow

CAA action workflow will be triggered on all Pull Request `opened, synchronize, closed`. This workflow will always run in the base repository and that's why we are making use of the [pull_request_target](https://docs.github.com/en/actions/reference/events-that-trigger-workflows#pull_request_target) event.
<br/> When the CAA workflow is triggered on pull request `closed` event, it will lock the Pull Request conversation after the Pull Request merge so that the contributors cannot modify or delete the signatures (Pull Request comment) later. This feature is optional.

#### 3. Signing the CAA

CAA workflow creates a comment on Pull Request asking contributors who have not signed  CAA to sign and also fails the pull request status check with a `failure`. The contributors are requested to sign the CAA within the pull request by copy and pasting **"I have read the CAA Document and I hereby sign the CAA"** as a Pull Request comment like below.
If the contributor has already signed the CAA, then the PR status will pass with `success`. <br/>

##### Demo for step 2 and 3

![signature-process](https://github.com/caa-assistant/github-action/blob/master/images/signature-process.gif?raw=true)

<br/>

#### 4. Signatures stored in a JSON file

After the contributor signed a CAA, the contributor's signature with metadata will be stored in a JSON file inside the repository and you can specify the custom path to this file with `path-to-signatures` input in the workflow. <br/> The default path is `path-to-signatures: 'signatures/version1/caa.json'`.
...
and `remote-repository-name`: `<your repo name>` in your CAA workflow file.
...
![signature-storage-file](https://github.com/caa-assistant/github-action/blob/master/images/signature-storage-file.gif?raw=true)

#### 5. Users and bots in allowlist

If a GitHub username is included in the allowlist, they will not be required to sign a CAA. You can make use of this feature If you don't want your colleagues working in the same team/organisation to sign a CAA. And also, since there's no way for bot users (such as Dependabot or Greenkeeper) to sign a CAA, you may want to add them in `allowlist`. You can do so by adding their names in a comma separated string to the `allowlist` input in the CAA  workflow file(in this case `dependabot[bot],greenkeeper[bot]`). You can also use wildcard symbol in case you want to allow all bot users something like `bot*`.

##### Demo for step 5

![allowlist](https://github.com/caa-assistant/github-action/blob/master/images/allowlist.gif?raw=true)
...
![personal-access-token](https://github.com/caa-assistant/github-action/blob/master/images/personal-access-token.gif?raw=true)
...
| --------------------- | ----------- | ----------- | ------- |
| `path-to-document`     | _required_ |  provide full URL `https://<caafile>` to the document which shall be signed by the contributor(s)  It can be any file e.g. inside the repository or it can be a gist. | https://github.com/caa-assistant/github-action/blob/master/SAPCAA.md |
| `path-to-signatures`       | _optional_ |  Path to the JSON file where  all the signatures of the contributors will be stored inside the repository. | signatures/version1/caa.json |
...
| `allowlist`   | _optional_ | You can specify users and bots to be [added in allowlist](https://github.com/caa-assistant/github-action#5-users-and-bots-in-allowlist).  | user1,user2,bot* |
...
| `create-file-commit-message`   | _optional_ |Commit message when a new CAA file is created. | Creating file for storing CAA Signatures. |
| `signed-commit-message`   | _optional_ | Commit message when a new contributor signs the CAA in a Pull Request. |  $contributorName has signed the CAA in $pullRequestNo |
| `custom-notsigned-prcomment`   | _optional_ | Introductory Pull Request comment to ask new contributors to sign. | Thank you for your contribution and please kindly read and sign our $pathToCAADocument |
| `custom-pr-sign-comment`   | _optional_ | The signature to be committed in order to sign the CAA. | I have read the Developer Terms Document and I hereby accept the Terms |
| `custom-allsigned-prcomment`   | _optional_ | pull request comment when everyone has signed | All Contributors have signed the CAA. |
