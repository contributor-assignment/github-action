...
export async function setupClaCheck() {
	...
	try {
		...
		if (reactedCommitters?.newSigned.length) {
			/* pushing the recently signed  contributors to the CAA Json File */
		...
		if (
			...
			core.info(`All contributors have signed the CAA 📝 ✅ `);
...
async function getCAAFileContentandSHA(
...
async function createClaFileAndPRComment(
	...
	throw new Error(
		`Committers of pull request ${context.issue.number} have to sign the CAA`,
	);
}
