...
export async function addEmptyCommit() {
	...
	core.info(
		`Adding empty commit for ${contributorName} who has signed the CAA `,
	);

	if (context.payload.comment) {
		//Do empty commit only when the contributor signs the CAA with the PR comment
		if (
			...
			try {
				const message = input.getSignedCommitMessage()
					...
					: ` @${contributorName} has signed the CAA `;
